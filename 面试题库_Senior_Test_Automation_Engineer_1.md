...
**候选人:** Hongwei Yin
**岗位:** Senior Test Automation Engineer(Trading & Financial Applications)
**面试官:** Test Manager
**建议时长:** 90 分钟(可拆为技术面 60 分钟 + 主管面 30 分钟)
...
## 附录 F — Python 代码能力评估(25 分钟,不考算法)

> **设计原则:** 测试工程师不需要手写算法。他需要的是:读懂别人的烂代码、看出可维护性问题、设计可复用结构、定位诡异 bug。下面三道题按这个逻辑设计。
> **执行方式:** 打印出来给他,或屏幕共享。允许边看边说,不要求写完整代码。
...
> *"This is from a real codebase. A junior engineer on your team submitted this as a merge request. **Review it for me — what would you comment on?**"*
>
> 追问:*"If you could only fix three things, which three, and why those?"*

---

#### 埋设的问题清单(共 13 项,用作打分勾选)

**第一层 —— 初级也能看出(看出 3-4 项 = 及格线)**

- [ ] **硬编码 URL** —— 没有环境配置,换环境就得改代码
- [ ] **Token 写死在源码里** —— 密钥进了代码仓库,而且永不刷新,过期即全挂
- [ ] **`time.sleep(5)`** —— 固定等待,慢时不够、快时浪费;应该轮询 + 超时
- [ ] **断言太弱** —— 只验状态码和一个字段,大量 bug 漏网
- [ ] **重复的 header 构造** —— 三处复制粘贴

**第二层 —— 中级水平(再看出 3-4 项)**

- [ ] **`global order_id` 造成用例间耦合** —— 三条用例必须按顺序跑,**单独跑第二条直接 NameError**
- [ ] **无法并行执行** —— 上一条的直接后果,这是规模化的致命伤
- [ ] **没有清理(teardown)** —— 每跑一次都在环境里留下脏订单,跑一百次环境就废了
- [ ] **没有 `timeout` 参数** —— 服务端 hang 住时用例永久挂起,整条流水线卡死
- [ ] **断言失败时没有上下文** —— 只报 `assert 200 == 500`,看不到响应体,排查必须重跑

**第三层 —— 高级水平(看出这几项 = Senior 实锤)**

- [ ] **`price` 用了 float** —— 金融金额用浮点数,精度问题迟早爆;应该用 `Decimal` 或整数最小单位。**这条是金融背景的分水岭**
- [ ] **业务逻辑自相矛盾** —— 限价单为什么必然 `FILLED`?价格设在这里未必成交。**更荒谬的是第三条:已经 FILLED 的订单还去撤单,这个用例本身就是错的**
- [ ] **这三个 "test" 不是测试,是步骤** —— 合起来才是一个场景。pytest 报告会显示三个独立用例,实际只是一个流程的三段,命名和结构都在误导人
...
| 分数 | 表现 |
|---|---|
| **5** | 看出 10 项以上;**主动指出业务逻辑矛盾(撤单已成交订单)和 float 精度问题**;能讲清"耦合→无法并行→无法规模化"的因果链;三项优先级选得有道理 |
| **4** | 看出 7-9 项,覆盖前两层,表达结构化 |
| **3** | 看出 4-6 项,基本是第一层,能说出"应该用 fixture" |
| **2** | 只看出 sleep 和硬编码 |
| **1** | 说"看起来没什么大问题" |

**特别注意:** "只修三个"这个追问在测 **Decision Quality**。好答案会区分**致命 vs 难看** —— token 泄露和用例耦合是致命的,重复代码只是难看。
...
> *"Take the `time.sleep(5)` — that's the one that bites us most in CI. **Write me a proper replacement.** A helper that waits for the order to reach a given status. Paper is fine, or just talk me through it."*

**期待的要素(勾选):**

- [ ] 轮询循环 + **总超时上限**(不是无限重试)
- [ ] 轮询间隔(固定可接受,**指数退避加分**)
- [ ] **超时后抛出有意义的错误** —— 包含期望状态、实际状态、等了多久
- [ ] 处理终态:订单已经 `REJECTED` 时**应立刻失败,而不是继续等到超时**(高分项,说明他真被坑过)
- [ ] 函数签名合理:目标状态、超时、间隔都可传入
- [ ] 网络异常要不要重试的考虑

一个合格答案大致长这样:

```python
def wait_for_status(client, order_id, expected, timeout=30, interval=0.5):
    deadline = time.monotonic() + timeout
    last = None
    while time.monotonic() < deadline:
        last = client.get_order(order_id)["status"]
        if last == expected:
            return
        if last in TERMINAL_STATES and last != expected:
            raise AssertionError(
                f"Order {order_id} reached terminal state {last}, "
                f"expected {expected}"
            )
        time.sleep(interval)
    raise TimeoutError(
        f"Order {order_id} still {last} after {timeout}s, expected {expected}"
    )
```

**不要求写得一模一样。** 看三件事:有没有 deadline、有没有终态短路、错误信息里有没有诊断信息。

---

### F.3 设计追问(5 分钟,口头即可)

> *"Now zoom out. This codebase has 300 tests written like this. **You're asked to clean it up. What's your plan for the first two weeks?**"*

**观察点:**
- 有没有**先止血再重构**的意识(先把 token 和 URL 挪出去,再动结构)
- 会不会提**先建立安全网**(现有用例虽烂,但改之前得能验证没改坏)
- 有没有提**分层**:HTTP 客户端层 / 业务动作层 / 断言层 / 用例层
- 会不会提 pytest fixture、conftest、参数化
- **有没有说"不全改"** —— 300 条一次性重构不现实,能说出"先改最常挂的那 30 条"是成熟信号

> 追问:*"You mentioned fixtures. How would you handle test data — each test creates its own account, or you share a pool of pre-created accounts? What breaks in each approach?"*

这题没标准答案,看他能否讲清**两种方案各自的代价**(独立造数据慢但干净;共享池快但有污染和并发冲突)。

---

### F.4 如果想看真实代码(可选)

他的浏览器书签里有 GitHub,但简历上没放链接。直接问:

> *"Do you have any code you can show me? A framework you built, or anything on GitHub?"*

看真实代码比任何面试题都准。如果他说是公司资产不能外传(**完全合理**),就问:

> *"Fair enough. Then describe the file structure of your automation framework — top level directories, what goes where."*

讲不出目录结构的人,大概率是在别人搭好的框架里写用例的,不是搭框架的人。
...
| 维度 | 观察点 | 1-5 |
|---|---|---|
| 代码阅读与问题识别 | F.1 看出几项、分几层 | |
| 优先级判断 | 致命 vs 难看的区分 | |
| 金融领域敏感度 | 是否指出 float 精度问题 | |
| 业务逻辑敏感度 | 是否发现撤单已成交订单的矛盾 | |
| 动手能力 | F.2 的 helper 质量 | |
| 框架设计思维 | F.3 的分层与止血顺序 | |
| 工程成熟度 | 是否主动说"不全改" | |

**与语言栈迁移的关联:** 如果他在 F.1 和 F.3 表现好,说明**他的设计判断力与语言无关**,迁移到 Java/TS 的风险显著降低。反过来,如果他只能靠 Python 语法糖答题、说不出通用设计原则,迁移风险就很高。**这两题要和 Part 3.4 合起来看。**

---

## 附录 G — API 测试用例设计实战题(真实接口,20 分钟)

> **说明:** 本题取自真实系统,已做脱敏(域名、IP、账户 ID 已替换)。**优势在于里面的问题是真实存在的,不是人为设计的** —— 候选人无法靠"面试题套路"作答。
> **执行方式:** 把下面「给候选人的材料」单独打印一页递给他,给 3 分钟阅读,然后开始提问。
...
Host: trade-uat.internal:8089
Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryXXXX
X-User-Id: maker@bank.com
```

**Part 1** — `name="trade"`, `filename="blob"`, `Content-Type: application/json`
...
**Part 2** — `name="datFile"`, `filename="FX_TRF.dat"`, `Content-Type: application/octet-stream`
        ...
        "TRF": {
          ...
          "payoffDict": {
            "1": {
              "PD_BrokenLine": {
                "pdbrokenLine": [
                  { "Segment": { "low": "-Infinity", "high": 146.2,
                                 "slope": -1, "xAnchor": 146.2, "yAnchor": 0,
                                 "inclLow": false, "inclHigh": false } },
                  { "Segment": { "low": 146.2, "high": "Infinity",
                                 "slope": -1, "xAnchor": 146.2, "yAnchor": 0,
...
> *"This is a real request and response from a trade booking system. FX Target Redemption Forward, saved by a maker.*
>
> ***Design the test cases. Walk me through how you'd approach it — what you'd test, and how you'd prioritise.***
>
> *Take a couple of minutes to read it first."*

**不要提示、不要引导。** 让他自己找。记录他提到的维度顺序 —— 先想到什么,反映他的思维习惯。

**第二段(7 分钟)— 定向追问**
他没覆盖到的维度,用下面 G.4 的问题挖。
...
- [ ] **`notionalCurrency` 是空字符串,却返回 SUCCESS** —— 外汇产品名义货币为空,系统凭什么接受?
- [ ] **`premiumAmount: 0` 配 `premiumCurrency: "USD"`** —— 零权利金合法吗?若合法,币种还有意义吗?
- [ ] **日期关系:** `novationDate` (09-14) 早于 `premiumDate` (09-16),这个顺序有约束吗?两者倒置会怎样?交易日期与系统业务日期的关系呢?
- [ ] **`tradeAction=SAVE` 暗示还有别的动作** —— SUBMIT / AMEND / DELETE / APPROVE?状态机怎么测?SAVE 之后能直接 DELETE 吗?
- [ ] **`udf` 是动态键值数组** —— 重复 key 会怎样?未知 key?`type` 与 `value` 不匹配(type=Date 但 value 是文本)?数组为空?
- [ ] **multipart 两部分的一致性** —— JSON 里说 FX_TRF,`.dat` 文件里是别的产品,系统以哪个为准?
- [ ] **幂等性** —— 同一请求发两次,是生成两笔交易还是被拦截?没有看到客户端幂等键。

#### 第三层 —— 高级(看出这几项 = Senior 实锤)

- [ ] **Novation 的对手方前后完全相同** —— `counterpartyFmId` / `counterpartyName` 与 `oldCptyFmId` / `oldCptyName` 一模一样(都是 300000001 / "10 AM NY")。**转让给自己的 novation 在业务上无意义,系统却接受了。** 这是全题最有价值的发现。

- [ ] **分段边界的包含性设计** —— 第一段 `high=146.2, inclHigh=false`,第二段 `low=146.2, inclLow=true`。**恰好在 146.2 这一点上,归属第二段,既不重叠也不留空。** 能主动指出"我要专门测边界点归属"的人,是真正的测试设计高手。
  - 追问:如果两段的 `incl` 都是 false,146.2 这个点属于谁?payoff 怎么算?
  - 追问:如果都是 true,重复计算怎么办?

- [ ] **`-Infinity` / `Infinity` 是字符串,而 `high` / `low` 的另一端是数字** —— 同一个字段在同一个对象里类型不一致。JSON 原生不支持 Infinity,只能字符串编码。**反序列化会不会出错?下游系统能不能处理?回写时能不能原样还原?**

- [ ] **`X-User-Id` 放在请求头里** —— 这是身份还是普通参数?**我能不能把它改成别人的邮箱,以别人的名义建单?** 银行系统的 maker-checker 隔离在这里能不能绕过?**Maker 能不能审批自己创建的交易?**

- [ ] **HTTP 200 与响应体里的 `code: 200` 双重状态** —— 业务失败时 HTTP 还是 200 吗?如果是,自动化断言只看 HTTP 状态码就会全部漏判。**这直接关系到测试框架怎么写断言。**

- [ ] **`dd_roundings: null`(实物交割)** —— 实物交割的取整规则为空,到期交割时数量怎么算?这是缺省值还是缺陷?

- [ ] **响应耗时约 60 秒** —— 创建一笔交易要一分钟。网关超时多少?客户端超时多少?**超时之后交易到底建了没有?会不会产生"前端报错但后台已建单"的幽灵交易?**

---

### G.4 定向追问(他没提到的维度用这些挖)

> *"You didn't mention the `udf` array. What could go wrong there?"*

> *"Look at the counterparty fields and the novation fields. Anything strike you as odd?"* ← 引导到第三层第一项

> *"Look at the two segments in `pdbrokenLine`. **What's the one price you'd absolutely make sure to test?**"* ← 期待答 146.2

> *"This response took about a minute. Does that change how you'd test it?"*

> *"The `X-User-Id` header — is that authentication, or just data?"*
...
> *"Say the payoff calculation comes back wrong. **How would you know?** Where does your expected value come from?"*

这题直通他简历上的 Black-Scholes 那条。**能讲清"不能用系统自己的逻辑当真值"的人,是 5 分。**

**杀手锏二 —— 只测十条**

> *"You've got half a day and you can only write ten test cases. **Which ten?** Tell me why those, and what risk you're accepting by leaving the rest out."*

看的不是选了哪十条,而是**他有没有风险分级的框架**,以及敢不敢明说"我放弃了什么"。同时取证 **Decision Quality** 和 **Courage**。

**杀手锏三 —— 分层归属**

> *"Of the cases you listed — which ones would you actually automate at the API layer, which belong in unit tests, and which need full end-to-end?"*

答"全部 API 层自动化"是 2 分。真正的高手会说:payoff 的数学计算属于单元测试,maker-checker 流程必须 E2E,字段校验放 API 层。
...
| 分数 | 表现 |
|---|---|
| **5** | 看出 12 项以上;**主动发现 novation 对手方相同的业务矛盾**;主动指出 146.2 边界点必须单独测;能讲清 HTTP 200 双状态对断言策略的影响;分层归属清晰 |
| **4** | 看出 9-11 项,覆盖前两层,有优先级思维 |
| **3** | 看出 5-8 项,以字段校验为主,业务维度薄 |
| **2** | 只做输入校验的罗列,看不出方法论 |
| **1** | 只能说"测正常流和异常流" |

---

### G.7 加分观察(与打分无关,但值得记录)

- **他有没有先问问题?** 真正的高手看到这份材料第一反应通常是:"FX TRF 的 target 条款在哪?这个 payload 里没看到累计目标和敲出条件。" —— **这确实是材料里缺的信息**,能问出来说明他真懂这个产品。
- **他会不会要求看 `.dat` 文件内容?** 会问的人,说明他知道 multipart 两部分的一致性是个风险点。
- **他对 `portfolioId: "ABS_EQF"` 有没有疑问?** 一个股票类组合(EQF)里放外汇产品(FX_TRF),合理吗?这是产品与组合的匹配校验。
